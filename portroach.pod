...
portroach - A tool to scan for new versions of FreeBSD ports.
...
The current version of Portroach is also capable of checking OpenBSD's ports,
...
Create database:
    ...
    # createuser -U pgsql -P portroach
    # createdb -U pgsql portroach
...
If you're using Portroach to check FreeBSD ports, run:
...
If you would like to check another software repository/source, Portroach
has several options.

Firstly, NetBSD and OpenBSD's ports trees are supported by the standard
"Ports" backend. See F<portroach.conf> for details on how to configure this.
Make sure you've got the correct C<make> at hand for Portroach if you're
...
that you want to manage by hand. See L<Portroach::DataSrc::XML|Portroach::DataSrc::XML>
...
backwards compatibility with the FreeBSD tree. The PORTSCOUT variable for use in
Makefile's remained unchanged.
...
=item
Copyright (C) 2014, Jasper Lievisse Adriaanse E<lt>jasper@humppa.nlE<gt>.
